...
// Prepare creates a prepared DELETE statement from the current generic query builder state.
...
// Example:
//
//	prepared, err := DeleteG[User]("users").
//		WithClient(client).
//		Where("status = ?", "inactive").
//		Prepare(ctx)
//	if err != nil {
...
func (q *DeleteQueryBuilderG[T]) Prepare(ctx context.Context) (*PreparedExec, error) {
