...
func ExampleParseWhere() {
	...
	expr1, err := sqlc.ParseWhere("id = 1 AND name = 'foo'")
	if err != nil {
		panic(err)
	}
	...
	// Example 2: IN clause
	expr2, err := sqlc.ParseWhere("status IN ('active', 'pending', 'approved')")
	if err != nil {
		panic(err)
	}
	...
	expr3, err := sqlc.ParseWhere("(age >= 18 AND age <= 65) OR status = 'vip'")
	if err != nil {
		panic(err)
	}
	...
	expr4, err := sqlc.ParseWhere("deleted_at IS NULL AND email IS NOT NULL")
	if err != nil {
		panic(err)
	}
	...
	expr5, err := sqlc.ParseWhere("name LIKE '%john%' AND email LIKE '%@example.com'")
	if err != nil {
		panic(err)
	}
	...
	expr6, err := sqlc.ParseWhere("NOT (deleted = 1 OR banned = 1) AND verified = 1")
	if err != nil {
		panic(err)
	}
