...
// Prepare creates a prepared UPDATE statement from the current generic query builder state.
...
// Example:
//
//	prepared, err := UpdateG[User]("users").
//		WithClient(client).
//		Set("last_seen", nil).
//		Where("id = ?", 0).
//		Prepare(ctx)
...
func (q *UpdateQueryBuilderG[T]) Prepare(ctx context.Context) (*PreparedExec, error) {
